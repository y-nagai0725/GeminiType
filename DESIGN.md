...
2.  **ユーザー登録画面:** 新規ユーザー登録を行う。
3.  **ログイン画面:** 登録済みユーザーのログイン処理を行う。
4.  **メインメニュー画面:** 「AI生成」と「DBジャンル」の選択。
5.  **タイピング設定画面:** 「問題数」「特殊モード」「音設定」を選択する画面。
6.  **タイピング実行画面:** 連続タイピングを行う画面。進捗も表示。**（※PCでの表示、実行を想定）**
...
11. **Not Found 画面:** 存在しないurlへのアクセス時に表示。
